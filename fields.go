...
func Bool(key string, value bool) FieldBuilder {
	...
	f.Field.setKey(key)
	if value {
		f.msg = append(f.msg, "true"...)
	} else {
		f.msg = append(f.msg, "false"...)
	}
	return FieldBuilder{field: f}
}
...
func Int64(key string, value int64) FieldBuilder {
	f := fastFieldPool.Get()
	f.Field.setKey(key)
	f.msg = append(f.msg, strconv.FormatInt(value, 10)...)
	return FieldBuilder{field: f}
}

func Uint(key string, value uint) FieldBuilder {
	f := fastFieldPool.Get()
	f.Field.setKey(key)
	f.msg = append(f.msg, strconv.FormatUint(uint64(value), 10)...)
	return FieldBuilder{field: f}
}

func Uint64(key string, value uint64) FieldBuilder {
	f := fastFieldPool.Get()
	f.Field.setKey(key)
	f.msg = append(f.msg, strconv.FormatUint(value, 10)...)
	return FieldBuilder{field: f}
}
...
func Float32(key string, value float32) FieldBuilder {
	f := fastFieldPool.Get()
	f.Field.setKey(key)
	f.msg = strconv.AppendFloat(f.msg, float64(value), 'g', -1, 32)
	return FieldBuilder{field: f}
}
...
func Float64(key string, value float64) FieldBuilder {
	f := fastFieldPool.Get()
	f.Field.setKey(key)
	f.msg = strconv.AppendFloat(f.msg, value, 'g', -1, 64)
	return FieldBuilder{field: f}
}
